# Author-Ross Korsky and Eirinn Mackay
# Version-1.2.0
# Released under the MIT license. See License.txt for full license information.
#
# Description-Generates a Kerf compensated DXF file to produce laser cut parts.
...
class DxfForLaserAddin(fission.CommandBase):
    ...
    def on_execute(self, args) -> 'execute':
        (component, selection_name, plane, offset_sketch, perimeter_sketch,
         cutouts_sketch, might_be_nonmanifold) = self.generate_offset_sketch(False)
        ...
        if perimeter_sketch and cutouts_sketch:
            ...
            if file_picker.showSave() == adsk.core.DialogResults.DialogOK:
                if self.split_into_layers.eval():
                    ...
                    perimeter_dxf_path = os.path.join(
                        tmp_dir.name, 'perimeter_sketch.dxf')
                    cutouts_dxf_path = os.path.join(
                        tmp_dir.name, 'cutouts_sketch.dxf')
    ...
    def generate_offset_sketch(self, is_preview):
        ...
        overall_clock = fission.Stopwatch()
        create_assets_clock = fission.Stopwatch()
        build_curve_lookup_clock = fission.Stopwatch()
        loop_perf_tracker = fission.RelayStopwatch()
        transcription_segment_tracker = fission.SegmentedStopwatch()
        finalize_clock = fission.Stopwatch()
        ...
        planeInput.setByOffset(face, fission.InputFactory._ValueInput(0))
        ...
        if nonmanifold_points:
            ...
            pos = fission.Point3D(min_x, max_y + size * 3.1)
            pos = under_warn_sketch.modelToSketchSpace(
                offset_sketch.sketchToModelSpace(pos))
            text_input = warn_sketch.sketchTexts.createInput(
                '''WARNING non-manifold solid.
            ...
            for p in nonmanifold_points:
                warn_circle = under_warn_sketch.sketchCurves.sketchCircles.addByCenterRadius(
                    under_warn_sketch.modelToSketchSpace(offset_sketch.sketchToModelSpace(fission.Point3D(p.x, p.y))), 0.05)
        ...
        for loop in face.loops:
            loop_perf_tracker.start_section(
                'loop with {} edges'.format(loop.edges.count))
            ...
            s_param = min_param + \
                ((max_param - min_param) *
                 (0.667 if co_edge.isParamReversed else 0.333))
            ...
            assert abs(
                lead_vector.z) < fission.SketchHelper.CLOSE_ENOUGH, 'expected edge-tangent vector to lay on sketch plane, but it did not!'

            # Rotate the vector "out" (clockwise) 90deg
            out_vector = adsk.core.Vector3D.create(
                lead_vector.y, -lead_vector.x, 0)
            if lead_vector.z != 0:
                out_vector.normalize()

            offset_v = out_vector.copy()
            # Negative offsets are now supported by Fusion!
            offset_v.scaleBy(abs(half_kerf))
            ...
            if loop.edges.count == 1:
                ...
                if curve.curveType == adsk.core.Curve3DTypes.Circle3DCurveType:
                    transcription_segment_tracker.switch_segment(
                        'Handle Circles')
                    ...
                    if loop_sketch_curves.count > 1:
                        loop_sketch_curves = target_sketch.findConnectedCurves(
                            loop_sketch_curves.item(0))
                    offset_curves = target_sketch.offset(loop_sketch_curves, target_sketch.modelToSketchSpace(
                        offset_sketch.sketchToModelSpace(offset_to)), half_kerf)
            ...
            # Try to find an existing curve as projected from the face when the sketch was created.
                ...
                transcription_segment_tracker.switch_segment(
                    'Find First Curve')
                first_curve = None
                (r, first_point) = edge_eval.getPointAtParameter(
                    max_param if co_edge.isOpposedToEdge else min_param)
                if r:
                ...
                if not first_curve:
                    ...
                    for co_edge in loop.coEdges:
                        if not first_curve:
                            first_curve = offset_sketch.project(
                                co_edge.edge).item(0)
                        else:
                ...
                loop_sketch_curves = offset_sketch.findConnectedCurves(
                    first_curve)
                transcription_segment_tracker.switch_segment(
                    'Offset Connected Curves')
                offset_curves = offset_sketch.offset(
                    loop_sketch_curves, offset_to, half_kerf)
                if not is_preview:
                    transcription_segment_tracker.switch_segment(
                        'Project Final Curves')
        ...
        if SHOW_DBG_PERFORMANCE_INFO:
            ...
            perf += 'Create Assets.....: {}\n'.format(
                create_assets_clock.elapsed)
            perf += 'Build Curve Lookup: {}\n'.format(
                build_curve_lookup_clock.elapsed)
            perf += 'Transcribe Loops\n{}\n'.format(
                re.sub('^', '  ', str(transcription_segment_tracker), flags=re.MULTILINE))
        ...
        if might_be_nonmanifold:
            self.ui.messageBox('''Encountered a problem with the selected face. The face may contain non-manifold geometry or other sketch projection artifacts.
            Please take a close look at the output sketches before cutting. Paths with a problem will appear as fixed sketch lines (green) on the "Path Offsets" sketch.
            Pay close attention to where arcs/curves intersect other edges.\n\nTip: Try selecting the other side of the same body instead.''')
    ...
    def initialize_inputs(self, factory):
        ...
        self.split_into_layers = factory.create_checkbox(
            ...
            description='''When set cutouts (holes) and the perimeter will be placed on separate layers (and separate colors) in the DXF file.
            This feature will allow you to improve the accuracy of your laser cut parts by cutting cutouts first and your perimeter last, if supported by your cutting software.''')
